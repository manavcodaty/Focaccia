...
const ticketsRoot = path.join(repoRoot, 'apps/tickets');
...
async function startTicketsDev() {
  ...
  const next = start('tickets web app', 'pnpm', [
    'exec',
    'next',
    'dev',
    '--hostname',
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? '').href) {
  startTicketsDev().catch((error) => {
    process.stderr.write(`${error instanceof Error ? error.message : 'Tickets dev startup failed.'}\n`);
    process.exitCode = 1;
  });
}
