...
- Local verification on this machine uses Colima, and Supabase CLI 2.75.0 can only start the stack reliably when Docker is addressed as `ssh://colima` instead of the default host Unix socket. The separate `supabase functions serve` process also required `--no-verify-jwt` because the bundled local edge runtime rejected the local HS256 access tokens before the request reached the function handlers; the handlers still enforce auth explicitly via Supabase Auth inside the protected functions.
