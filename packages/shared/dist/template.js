...
exports.cancelableTemplateV1 = cancelableTemplateV1;
...
async function cancelableTemplateV1(embedding, eventSalt) {
    ...
    const hashView = new DataView(hashInput.buffer, hashInput.byteOffset + EVENT_SALT_BYTES, 4);
    hashInput.set(eventSalt, 0);
    try {
        for (let bitIndex = 0; bitIndex < TEMPLATE_BITS; bitIndex += 1) {
            let sum = 0;
            hashView.setUint16(0, bitIndex, false);
            for (let dimensionIndex = 0; dimensionIndex < normalizedEmbedding.length; dimensionIndex += 1) {
